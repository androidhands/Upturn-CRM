...
export default function Edit({ auth, department,company }) {
   ...
   const onSubmit = (e) => {
      ...
      put(route('department.update',[company.id, department.id]));
   }

   const breadcrumbs = [
      { name: company.name, href: route('company.show', company.id) },
      { name: 'Business Units', href: route('department.index', company.id) },
      { name: `Edit ${department.name}`, href: route('department.edit', [company.id, department.id]) },
