...
export default function Edit({ auth, businessUnit,company }) {
   ...
   const onSubmit = (e) => {
      ...
      put(route('businessUnit.update',[company.id, businessUnit.id]));
   }

   const breadcrumbs = [
      { name: company.name, href: route('company.show', company.id) },
      { name: 'Business Units', href: route('businessUnit.index', company.id) },
      { name: `Edit ${businessUnit.name}`, href: route('businessUnit.edit', [company.id, businessUnit.id]) },
